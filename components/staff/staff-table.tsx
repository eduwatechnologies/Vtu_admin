...
import { toggleStaffStatus, logActivity } from "@/lib/redux/slices/staffSlice";
...
export function StaffTable() {
  ...
  const getRoleName = (roleId: string) => {
    return roles.find((role) => role._id === roleId)?.name || "Unknown Role";
  };
  ...
  const handleStatusChange = (staffId: string, currentStatus: string) => {
    ...
    setActionType(currentStatus === "active" ? "deactivate" : "activate");
    setConfirmDialogOpen(true);
  };

  const confirmStatusChange = () => {
    if (selectedStaffId) {
      dispatch(toggleStaffStatus(selectedStaffId));

      // Log the activity
      const staff = filteredStaff.find((s) => s._id === selectedStaffId);
      if (staff && session?.user) {
        dispatch(
          logActivity({
