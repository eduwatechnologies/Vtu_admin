...
export const addRole = createAsyncThunk(
  "roles/addRole",
  async (roleData: Omit<Role, "id" | "createdAt" | "updatedAt">, thunkAPI) => {
    try {
...
export const logActivity = createAsyncThunk(
  "logs/logActivity",
  async (logData: Omit<ActivityLog, "id" | "timestamp">, thunkAPI) => {
    try {
    ...
    toggleStaffStatus: (state, action: PayloadAction<string>) => {
      ...
      if (filteredStaff) {
        filteredStaff.status =
          filteredStaff.status === "active" ? "inactive" : "active";
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateStaffMember.fulfilled, (state, action) => {
        ...
        // Also update in filtered staff
        const filteredIndex = state.filteredStaff.findIndex(
